...
export default function ProjectDisplay({
	...
	return (
		<div className={`group relative ${className} hover:z-10`}
			style={{transform: `rotateY(${tiltX * 10}deg) rotateX(${tiltY * 10}deg)`}}
			onMouseMove={onHover} onMouseLeave={onHoverEnd} {...rest}>
			<Link href={projectId ? `/work/${projectId}` : ''} className={`
				relative h-[260px] ${small ? '' : 'md:h-[536px]'} xs:min-w-[360px]
				overflow-clip
				...
				shadow-3xl shadow-black/80 group-hover:shadow-highlight/20
				transition-shadow duration-300

				${small ? '' : 'group-hover:overflow-visible'}`}

				// @ts-ignore
				style={{transformStyle: 'preserve-3d', '--color-highlight': `var(--color-${color})`}}
				>
				<div className={`
			...
			<section className={`
				absolute ${small ? 'inset-0.5 rounded-xl' : 'bottom-0.5 inset-x-0.5'} z-10
				p-8
				flex flex-col gap-1
				...
				<div className="h-[24px] flex gap-2">
					{
						techTags.map(tag => (
							<TechIcon key={tag} icon={`tech-${tag}` as IconNameTech} />
						))
					}
				</div>
...
function ProjectActionButton({href="", icon, children}: PropsWithChildren<{href?: string, icon?: IconName}>) {
	...
	return (
		<Link href={href} className="group/action py-1.5 flex flex-col items-center font-medium cursor-pointer hover:text-blue-200" target="_blank">
